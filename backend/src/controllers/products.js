...
export const createProduct = function (req, res) {
    ...
    db.query(query, [product_name, created_by, price, image_url], function (err, results) {
        if (err) {
            return res.status(500).send("Error creating product");
        }
        return res.status(201).json({ product_id: results.insertId, product_name, price, image_url });
    });
